...
var _drgData=null,_localFeedback=null;  // Full DRG data + local hospital feedback data

// ===== LOADING STATUS DIAGNOSTIC =====
var _loadState={js:0,icd10:0,icd9:0,drg:0,excl:0,proc:0,drug:0,tech:0};
function _markReady(id){
  _loadState[id]=1;
  var map={'JS':'stJS','ICD10':'stICD10','ICD9':'stICD9','DRG':'stDRG','Excl':'stExcl','Proc':'stProc','Drug':'stDrug','Tech':'stTech'};
  var elId=map[id.toUpperCase()]||('st'+id.charAt(0).toUpperCase()+id.slice(1));
  var el=document.getElementById(elId);
  if(el){el.style.color='#3ba06a';el.textContent='✅ '+el.textContent.replace('✅ ','').replace('⏳ ','');}
  var allReady=_loadState.js&&_loadState.icd10&&_loadState.icd9&&_loadState.drg&&_loadState.excl&&_loadState.proc;
  if(allReady){
    var re=document.getElementById('stReady');
    if(re){re.style.color='#3ba06a';re.textContent='✅ 就绪 — 实时转换已启用';}
  }
}
// Mark JS loaded
(function(){
  var el=document.getElementById('stJS');
  if(el){el.style.color='#3ba06a';el.textContent='✅ JS';}
  _loadState.js=1;
})();
...
// Load full DRG data for grouping recommendations
fetch('drg.json').then(r=>r.json()).then(d=>{
  _drgData=d;
  console.log('DRG data loaded: dx2a='+Object.keys(d.dx2a||{}).length+' px2a='+Object.keys(d.px2a||{}).length+' adrg='+(d.adrg?Object.keys(d.adrg).length:0));
}).catch(function(e){console.log('DRG data load skipped:',e.message);});

// Load local hospital feedback data (reverse-engineered weight table)
fetch('local_feedback.json').then(r=>r.json()).then(d=>{
  _localFeedback=d;
  console.log('Local feedback loaded: '+d.cases.length+' cases, '+d.groups.length+' groups');
}).catch(function(e){console.log('Local feedback not loaded (optional):',e.message);});

// Load drug audit rules for smart auditing
var _drugRules=null;  // {drug_name -> [{rule_type, drug_name, drug_code, check_logic, logic_basis, limit_type, insurance_note}]}
var _drugLimitMap=null; // {drug_name -> limit_type string}
fetch('drug_audit_index.json').then(function(r){return r.json();}).then(function(d){
  _drugRules=d.rules;
  _drugLimitMap=d.limit_map;
  console.log('Drug audit rules loaded: '+Object.keys(d.rules).length+' drugs');
  _markReady('drug');
}).catch(function(e){console.log('Drug audit rules load skipped:',e.message);});

// Load pricing guide tech_specs for decomposition detection
var _techSpecs=null;  // {code -> {name, desc}}
fetch('pricing_guide_tech.json').then(function(r){return r.json();}).then(function(d){
  _techSpecs=d;
  console.log('Tech specs loaded: '+Object.keys(d).length);
  _markReady('tech');
}).catch(function(e){console.log('Tech specs load skipped:',e.message);});
...
function _convert(){
  ...
  // DRG grouping recommendation
  var drgHtml=_recommendDRG(mainDx.code,filteredOtherDxs.map(function(d){return d.code}),mainProc.code,otherProcs.map(function(p){return p.code}),gender,age);
  if(drgHtml){
    document.getElementById('rx_specialList').innerHTML=drgHtml;
    document.getElementById('rx_specialSection').style.display='block';
  } else {
    document.getElementById('rx_specialSection').style.display='none';
  }

  // ===== SMART AUDIT PANEL =====
  var auditCards=_runSmartAudit(mainDx,filteredOtherDxs,mainProc,otherProcs,age,gender);
  if(auditCards.length>0){
    var gridHtml='';
    for(var gi=0;gi<auditCards.length;gi++){
      var card=auditCards[gi];
      var hdrColor=card.color||'#8b919e';
      var hdrBg=card.bg||'#fafbfc';
      gridHtml+='<div class="audit-card">';
      gridHtml+='<div class="audit-card-header" style="background:'+hdrBg+';color:'+hdrColor+'">'+card.icon+' '+card.title+'<span class="audit-count">'+card.rules.length+'条</span></div>';
      gridHtml+='<div class="audit-card-body">';
      for(var ri=0;ri<card.rules.length;ri++){
        var rule=card.rules[ri];
        var badgeCls=rule.sev==='intercept'?'badge-intercept':(rule.sev==='warn'?'badge-warn':'badge-info');
        var badgeTxt=rule.sev==='intercept'?'🚫拦截':(rule.sev==='warn'?'⚠️警告':'ℹ️提示');
        gridHtml+='<div class="audit-rule"><span class="audit-badge '+badgeCls+'">'+badgeTxt+'</span><span>'+rule.text+'</span></div>';
      }
      gridHtml+='</div></div>';
    }
    document.getElementById('rx_auditGrid').innerHTML=gridHtml;
    var totals={intercept:0,warn:0,info:0};
    auditCards.forEach(function(c){c.rules.forEach(function(r){totals[r.sev==='intercept'?'intercept':(r.sev==='warn'?'warn':'info')]++;})});
    document.getElementById('rx_auditSummary').innerHTML='<b>医保智能审核命中 '+totals.intercept+totals.warn+totals.info+' 条规则</b>（拦截'+totals.intercept+' / 警告'+totals.warn+' / 提示'+totals.info+')';
    document.getElementById('rx_auditSection').style.display='block';
  } else {
    document.getElementById('rx_auditSection').style.display='none';
  }
...
// ===== SMART AUDIT ENGINE (医保智能审核规则匹配) =====
function _runSmartAudit(mainDx, otherDxs, mainProc, otherProcs, age, gender){
  var result={total:0,intercept:0,warn:0,info:0,drugHtml:'',billingHtml:'',codingHtml:''};

  // ──── DOMAIN 1: 合理用药审核 ────
  // Note: This converter page focuses on diagnosis/procedure coding, not drug orders.
  // We still check if any drug-related audit rules apply via diagnosis-drug indication matching.
  if(_drugRules){
    var drugWarnings=[];
    // Check limit_type rules against patient age/gender
    if(_drugLimitMap){
      for(var lk in _drugLimitMap){
        var lt=_drugLimitMap[lk];
        if(lt.indexOf('限儿童专用')>=0 && age>=18){
          drugWarnings.push({sev:'warn',text:'【'+lk+'】限儿童专用 — 患者已'+age+'岁，不符合使用条件'});
        }
        if(lt.indexOf('限二级医院')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限二级医院使用 — 请确认就诊机构级别是否符合'});
        }
        if(lt.indexOf('限工伤保险')>=0){
          drugWarnings.push({sev:'warn',text:'【'+lk+'】限工伤保险支付 — 非工伤患者此药品需自费'});
        }
        if(lt.indexOf('限生育保险')>=0 && gender==='M'){
          drugWarnings.push({sev:'intercept',text:'【'+lk+'】限生育保险 — 男性患者不能使用，医保不予支付'});
        }
        if(lt.indexOf('限肿瘤')>=0){
          // Check if any diagnosis is a tumor
          var hasTumor=false;
          for(var ti=0;ti<otherDxs.length;ti++){
            var tc=otherDxs[ti].code;
            if((tc>='C00'&&tc<'C97')||(tc>='D00'&&tc<'D49')){hasTumor=true;break;}
          }
          if(!hasTumor){
            drugWarnings.push({sev:'warn',text:'【'+lk+'】限肿瘤用药 — 当前诊断无肿瘤编码，不符合医保支付条件'});
          }
        }
        if(lt.indexOf('限中医医院')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限中医医院使用 — 非中医医院开具此药品可能无法报销'});
        }
        if(lt.indexOf('限急诊')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限急诊使用 — 非急诊情况下使用此药品可能影响报销'});
        }
        if(lt.indexOf('限住院')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限住院使用 — 门诊患者使用此药品可能无法报销'});
        }
        if(lt.indexOf('限化疗')>=0){
          // Check if any diagnosis suggests cancer
          var hasCancer=false;
          for(var tj=0;tj<otherDxs.length;tj++){
            var tc2=otherDxs[tj].code;
            if(tc2>='C00'&&tc2<'C97'){hasCancer=true;break;}
          }
          if(!hasCancer){
            drugWarnings.push({sev:'warn',text:'【'+lk+'】限化疗用药 — 当前诊断无恶性肿瘤编码，不符合使用条件'});
          }
        }
        if(lt.indexOf('限检测结果')>=0 || lt.indexOf('限检验结果')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限检测结果 — 使用前需有相应的实验室检测指标支持'});
        }
        if(lt.indexOf('限适应症')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限适应症 — 需确认当前诊断在该药品的医保限定适应症范围内'});
        }
        if(lt.indexOf('限中医辩证')>=0){
          drugWarnings.push({sev:'info',text:'【'+lk+'】限中医辨证 — 需有中医师辨证论治记录'});
        }
      }
    }
    // Render drug audit results
    if(drugWarnings.length>0){
      var seen=new Set();
      var uniqueWarnings=[];
      for(var wi=0;wi<drugWarnings.length;wi++){
        var w=drugWarnings[wi];
        var key=w.text;
        if(!seen.has(key)){
          seen.add(key);
      ...
      result.drugHtml+='<div class="audit-panel audit-drug">';
      result.drugHtml+='<div class="audit-panel-header">💊 合理用药审核（'+uniqueWarnings.length+'条）</div>';
      result.drugHtml+='<div class="audit-panel-body">';
      for(var wj=0;wj<uniqueWarnings.length;wj++){
        var uw=uniqueWarnings[wj];
        var sevCls=uw.sev==='intercept'?'sev-intercept':(uw.sev==='warn'?'sev-warn':'sev-info');
        var sevLabel=uw.sev==='intercept'?'🚫 拦截':(uw.sev==='warn'?'⚠️ 警告':'ℹ️ 提示');
        result.drugHtml+='<div class="audit-item"><span class="audit-severity '+sevCls+'">'+sevLabel+'</span><span>'+uw.text+'</span></div>';
        result.total++;
        if(uw.sev==='intercept')result.intercept++;else if(uw.sev==='warn')result.warn++;else result.info++;
      }
      result.drugHtml+='</div></div>';
    }
  }

  // ──── DOMAIN 2: 收费合规审核 ────
  // Check for potential decomposition billing via tech_specs "含/不含" relationships
  if(_techSpecs){
    var billingWarnings=[];
    // Collect all procedure codes and names
    var allProcs=[mainProc].concat(otherProcs);
    // Build a set of matched tech spec codes
    var matchedSpecs=[];
    for(var pi=0;pi<allProcs.length;pi++){
      var pc=allProcs[pi].code;
      if(pc&&_techSpecs[pc]){
        matchedSpecs.push({code:pc,spec:_techSpecs[pc],proc:allProcs[pi]});
      }
    }
    // Check "不含" relationships: if a procedure says "不含XXX" and XXX is billed separately
    for(var si=0;si<matchedSpecs.length;si++){
      var spec=matchedSpecs[si].spec;
      var desc=spec.desc||'';
      // Find "不含" clauses
      var不含Match=desc.match(/不含([^。；、]+)[。；]/g);
      if(!不含Match)不含Match=desc.match(/不含([^。；]+)$/g);
      if(不含Match){
        for(var ni=0;ni<不含Match.length;ni++){
          var clause=不含Match[ni].replace(/不含/g,'').replace(/[。；、]+$/,'').trim();
          if(clause.length<2)continue;
          // Check if any other billed procedure matches this excluded item
          for(var pj=0;pj<allProcs.length;pj++){
            var other=allProcs[pj];
            if(other.code&&_techSpecs[other.code]){
              var otherSpec=_techSpecs[other.code];
              // If the excluded clause name appears in another billed procedure's name
              if(otherSpec.name.indexOf(clause)>=0 || other.name.indexOf(clause)>=0){
                billingWarnings.push({sev:'warn',text:'【'+spec.name+'】技术规范注明"不含'+clause+'"，但已单独计费【'+other.name+'('+(other.code||'')+')】— 涉嫌分解收费，应将该项目包含在内不再单独计费'});
              }
            }
          }
        }
    ...
    // Check for duplicate billing (same procedure billed multiple times)
    var procCodeCount={};
    for(var di=0;di<allProcs.length;di++){
      var dc=allProcs[di].code;
      if(dc){
        if(!procCodeCount[dc])procCodeCount[dc]=[];
        procCodeCount[dc].push(allProcs[di].name);
      }
    }
    for(var ck in procCodeCount){
      if(procCodeCount[ck].length>1){
        billingWarnings.push({sev:'intercept',text:'【重复收费】同一手术编码 '+ck+' 出现'+procCodeCount[ck].length+'次：'+procCodeCount[ck].join('、')+' — 医保审核重点监控'});
      }
    }
    // Render billing audit results
    if(billingWarnings.length>0){
      result.billingHtml+='<div class="audit-panel audit-billing">';
      result.billingHtml+='<div class="audit-panel-header">💰 收费合规审核（'+billingWarnings.length+'条）</div>';
      result.billingHtml+='<div class="audit-panel-body">';
      for(var bi=0;bi<billingWarnings.length;bi++){
        var bw=billingWarnings[bi];
        var sevCls=bw.sev==='intercept'?'sev-intercept':(bw.sev==='warn'?'sev-warn':'sev-info');
        var sevLabel=bw.sev==='intercept'?'🚫 拦截':(bw.sev==='warn'?'⚠️ 警告':'ℹ️ 提示');
        result.billingHtml+='<div class="audit-item"><span class="audit-severity '+sevCls+'">'+sevLabel+'</span><span>'+bw.text+'</span></div>';
        result.total++;
        if(bw.sev==='intercept')result.intercept++;else if(bw.sev==='warn')result.warn++;else result.info++;
      }
      result.billingHtml+='</div></div>';
    }
  }
  ...
  // Rule 1: 主要诊断选择规则 — 肿瘤化疗/放疗优先
  if(mainDx.code&&(mainDx.code.startsWith('C')||mainDx.code.startsWith('D0')||mainDx.code.startsWith('D1')||mainDx.code.startsWith('D2')||mainDx.code.startsWith('D3'))){
    for(var ci=0;ci<otherDxs.length;ci++){
      var od=otherDxs[ci];
      if(od.code.startsWith('Z51.0')){
        codingWarnings.push({sev:'warn',text:'【主要诊断选择】肿瘤患者行放射治疗(Z51.0)，医保清单建议优先选择Z51.0作为主要诊断，原肿瘤编码作为其他诊断'});
      }
      if(od.code.startsWith('Z51.1')||od.code.startsWith('Z51.2')){
        codingWarnings.push({sev:'warn',text:'【主要诊断选择】肿瘤患者行化学治疗(Z51.1/Z51.2)，医保清单建议优先选择Z51.1/Z51.2作为主要诊断，原肿瘤编码作为其他诊断'});
      }
    }
  }

  // Rule 2: 疑似诊断不得作为主要诊断
  var mainName=(mainDx.name||'').toLowerCase();
  if(mainName.indexOf('待查')>=0||mainName.indexOf('疑似')>=0||mainName.indexOf('待排')>=0||mainName.indexOf('?')>=0){
    codingWarnings.push({sev:'intercept',text:'【编码规范】主要诊断 "'+mainDx.name+'" 含疑似/待查表述 — 医保结算清单不允许使用疑似诊断作为主要诊断'});
  }

  // Rule 3: 术后并发症编码提示 (T81 category)
  for(var cj=0;cj<otherDxs.length;cj++){
    var od2=otherDxs[cj];
    if(od2.code&&od2.code.startsWith('T81')){
      codingWarnings.push({sev:'warn',text:'【编码规范】术后并发症 '+od2.code+' '+od2.name+' — 医保审核重点监控，须确认是否为医疗质量问题导致'});
    }
  }

  // Rule 4: 诊断编码特异性检查 — 如果诊断编码只有3位且后面没有扩展
  for(var ck2=0;ck2<otherDxs.length;ck2++){
    var code2=otherDxs[ck2].code;
    if(code2&&/^[A-Z][0-9]{2}$/.test(code2)){
      codingWarnings.push({sev:'info',text:'【编码规范】诊断 '+code2+' 仅3位，建议使用更具体的4-5位编码以提高编码特异性'});
    }
  }

  // Rule 5: 主诊断编码特异性
  if(mainDx.code&&/^[A-Z][0-9]{2}$/.test(mainDx.code)){
    codingWarnings.push({sev:'info',text:'【编码规范】主要诊断 '+mainDx.code+' 仅3位，建议使用更具体的编码'});
  }

  // Rule 6: 产科分娩编码优先级
  if(mainDx.code&&mainDx.code>='O10'&&mainDx.code<='O99'){
    if(!(mainDx.code>='O80'&&mainDx.code<='O84')){
      codingWarnings.push({sev:'warn',text:'【编码规范】产科诊断，医保结算清单要求分娩编码(O80-O84)优先作为主要诊断，当前主诊 '+mainDx.code+' 可能影响DRG分组'});
    }
  }

  // Rule 7: 无主要诊断
  if(!mainDx.code){
    codingWarnings.push({sev:'intercept',text:'【编码规范】缺少主要诊断编码 — 医保结算清单必须填写主要诊断'});
  }

  // Render coding audit results
  if(codingWarnings.length>0){
    result.codingHtml+='<div class="audit-panel audit-coding">';
    result.codingHtml+='<div class="audit-panel-header">📋 编码规范审核（'+codingWarnings.length+'条）</div>';
    result.codingHtml+='<div class="audit-panel-body">';
    for(var co=0;co<codingWarnings.length;co++){
      var cw=codingWarnings[co];
      var sevCls=cw.sev==='intercept'?'sev-intercept':(cw.sev==='warn'?'sev-warn':'sev-info');
      var sevLabel=cw.sev==='intercept'?'🚫 拦截':(cw.sev==='warn'?'⚠️ 警告':'ℹ️ 提示');
      result.codingHtml+='<div class="audit-item"><span class="audit-severity '+sevCls+'">'+sevLabel+'</span><span>'+cw.text+'</span></div>';
      result.total++;
      if(cw.sev==='intercept')result.intercept++;else if(cw.sev==='warn')result.warn++;else result.info++;
    }
    result.codingHtml+='</div></div>';
  }

  return result;
}

// ===== DRG GROUPING RECOMMENDATION (CHS-DRG 2.0 rules) =====
function _recommendDRG(mainDxCode, otherDxCodes, mainProcCode, otherProcCodes, gender, age){
  if(!_drgData||!mainDxCode)return '';
  var dd=_drgData;

  // ── Tumor type classification ──
  function tumorType(code){
    if(!code)return null;
    var c=code.toUpperCase();
    if(/^C[0-9]/.test(c)||/^D3[7-9]/.test(c)||/^D4[0-8]/.test(c))return 'malignant';
    if(/^D0[0-9]/.test(c))return 'insitu';
    if(/^D1[0-9]/.test(c)||/^D2[0-9]/.test(c)||/^D3[0-6]/.test(c))return 'benign';
    return null;
  }
  // Check if ADRG name/nature aligns with tumor type
  function adrgTumorAlign(adrgName, tt){
    if(!tt||!adrgName)return 0; // neutral
    var n=adrgName;
    if(tt==='malignant'){if(n.indexOf('恶性')>=0||n.indexOf('化学')>=0||n.indexOf('放射')>=0)return 1;if(n.indexOf('良性')>=0)return -1;}
    if(tt==='benign'){if(n.indexOf('良性')>=0)return 1;if(n.indexOf('恶性')>=0)return -1;}
    return 0;
  }

  // Diagnosis lookup with prefix fallback, returns {matches, spec: 0=exact|1=x001|2=broad}
  function lookDx(code){
    var m=dd.dx2a[code];if(m)return {m:m,spec:0};
    var b=code.match(/^([A-Z]\d{2}\.\d+)/);if(b){
      m=dd.dx2a[b[1]+'x001'];if(m)return {m:m,spec:1};
      m=dd.dx2a[b[1]];if(m)return {m:m,spec:2};
    }
    m=dd.dx2a[code+'x001'];return {m:m||[],spec:1};
  }

  // Procedure lookup
  function lookPx(code){
    var m=dd.px2a[code];if(m)return m;
    if(code.indexOf('.')>0){var p=code.split('.');m=dd.px2a[p[0]+'.'+p[1].substring(0,2)];if(m)return m;}
    return [];
  }

  // Collect all ADRG matches
  var adrgMap={};
  function add(result,source,isPD,spec){
    var matches=Array.isArray(result)?result:(result.m||[]);
    for(var i=0;i<matches.length;i++){
      var m=matches[i],k=m.a;
      if(!adrgMap[k])adrgMap[k]={a:m.a,an:m.an,d:m.d||[],matched:[],fromPD:false,fromPP:false,pdSpec:99};
      if(adrgMap[k].matched.indexOf(source)<0)adrgMap[k].matched.push(source);
      if(isPD&&spec<adrgMap[k].pdSpec)adrgMap[k].pdSpec=spec;
    }
  }
  var pdResult=lookDx(mainDxCode);add(pdResult,'主诊:'+mainDxCode,true,pdResult.spec);
  for(var k in adrgMap)adrgMap[k].fromPD=true;
  var ppMatches=mainProcCode?lookPx(mainProcCode):[];add(ppMatches,'主手术:'+mainProcCode,false,0);
  for(var k2 in adrgMap)adrgMap[k2].fromPP=true;
  for(var o=0;o<otherDxCodes.length;o++){var odr=lookDx(otherDxCodes[o]);add(odr,'其他诊:'+otherDxCodes[o],false,0);}
  for(var p=0;p<otherProcCodes.length;p++){var opm=lookPx(otherProcCodes[p]);add(opm,'其他手术:'+otherProcCodes[p],false,0);}

  // Determine MDC from primary diagnosis and procedure
  var dxMDC='',pxMDC='';
  if(pdResult.m&&pdResult.m.length>0){var mdcRaw=dd.adrg_mdc[pdResult.m[0].a];dxMDC=mdcRaw?mdcRaw.mdc:'';}
  if(ppMatches.length>0){var mdcRaw2=dd.adrg_mdc[ppMatches[0].a];pxMDC=mdcRaw2?mdcRaw2.mdc:'';}
  var isQY=!!(dxMDC&&pxMDC&&dxMDC!==pxMDC);

  // Category priority: 外科=0, 操作=1, 内科=2, unknown=3
  function catOrder(cat){return cat.indexOf('外科')>=0?0:(cat.indexOf('操作')>=0?1:(cat.indexOf('内科')>=0?2:3));}

  // Tumor type of primary diagnosis
  var tt=tumorType(mainDxCode);

  // Build candidate list
  var candidates=[];
  for(var ak in adrgMap){
    var a=adrgMap[ak],md=dd.adrg_mdc[a.a]||{};
    var cat=md.cat||'';var mdc=md.mdc||'';
    var mdcTier=(mdc===dxMDC)?0:((mdc===pxMDC&&isQY)?1:2);
    // Tumor alignment score: +2 for aligned, -2 for misaligned, 0 for neutral
    var taScore=adrgTumorAlign(a.an,tt);
    // Primary match flag: ADRG matched by main diagnosis or main procedure
    var fromPrimary=a.fromPD||a.fromPP;
    candidates.push({adrg:a,md:md,mdcTier:mdcTier,cat:cat,co:catOrder(cat),ta:taScore,fromPrimary:fromPrimary});
  }

  // Sort: primary match first, then tumor alignment, then MDC tier, then category, then specificity, then matches
  candidates.sort(function(a,b){
    if(a.fromPrimary!==b.fromPrimary)return b.fromPrimary-a.fromPrimary;  // Primary dx/px match first
    if(a.ta!==b.ta)return b.ta-a.ta;                                    // Aligned tumor type first
    if(a.mdcTier!==b.mdcTier)return a.mdcTier-b.mdcTier;                // Same MDC first
    if(a.co!==b.co)return a.co-b.co;                                     // 外科 > 操作 > 内科
    if(a.adrg.pdSpec!==b.adrg.pdSpec)return a.adrg.pdSpec-b.adrg.pdSpec; // More specific PD match first
    if(a.adrg.fromPP!==b.adrg.fromPP)return b.adrg.fromPP-a.adrg.fromPP;
    if(a.adrg.fromPD!==b.adrg.fromPD)return b.adrg.fromPD-a.adrg.fromPD;
    return b.adrg.matched.length-a.adrg.matched.length;
  });

  // MCC/CC detection from other diagnoses
  var mccSet=new Set(dd.mcc||[]),ccSet=new Set(dd.cc||[]),mccT=dd.mcc_tbl||{},ccT=dd.cc_tbl||{},exclT=dd.excl||{};
  var mccDx=[],ccDx=[],exDx=[];
  for(var d=0;d<otherDxCodes.length;d++){
    var dc=otherDxCodes[d];
    if(mccSet.has(dc)){var tbl=mccT[dc]||'',exCodes=exclT[tbl]||[];if(exCodes.indexOf(mainDxCode)<0)mccDx.push(dc);else exDx.push(dc);}
    else if(ccSet.has(dc)){var tbl2=ccT[dc]||'',exCodes2=exclT[tbl2]||[];if(exCodes2.indexOf(mainDxCode)<0)ccDx.push(dc);else exDx.push(dc);}
  }
  var mccLevel=mccDx.length>0?'MCC':(ccDx.length>0?'CC':'noCC');

  // === LOCAL FEEDBACK CORRECTION (文章方案: 历史医保反推校正) ===
  // 国家分组规则不变，仅在 MCC/CC 判定和 DRG 选择时引入本院历史经验校正
  var localBoost = 0;  // 历史本院实际入组的校正偏移
  if(_localFeedback && _localFeedback.cases){
    // 查找相似历史病例：主诊编码前缀相同 + 有/无手术
    var mainDxPrefix = mainDxCode.substring(0, 4); // e.g. D25.
    var hasMainProc = !!mainProcCode;
    var similarCases = _localFeedback.cases.filter(function(c){
      if(c.main_dx_prefix !== mainDxPrefix) return false;
      if(c.has_proc !== hasMainProc) return false;
      return true;
    });
    if(similarCases.length >= 3){ // 至少3例才有统计意义
      // 统计本院历史中 MCC/CC 的实际判定分布
      var histMCC = 0, histCC = 0, histnoCC = 0;
      similarCases.forEach(function(c){
        if(c.hist_mcc_level === 'MCC') histMCC++;
        else if(c.hist_mcc_level === 'CC') histCC++;
        else histnoCC++;
      });
      var total = histMCC + histCC + histnoCC;
      // 如果本院历史 MCC 比例 > 50%，说明本院实际 MCC 判定比国标更宽松
      // 给 MCC 候选加权重
      if(histMCC / total > 0.5) localBoost = 1;
      else if(histCC / total > 0.5) localBoost = 0;
      else localBoost = -1;
    }
  }

  // Build HTML - top 3
  var top3=candidates.slice(0,3),html='';
  for(var t=0;t<top3.length;t++){
    var c=top3[t],cat=c.cat||'';
    var catDisplay=cat.indexOf('外科')>=0?'外科':(cat.indexOf('操作')>=0?'操作':'内科');
    var catColor=catDisplay==='外科'?'#e85555':(catDisplay==='操作'?'#e8963a':'#4f8cf7');
    var drgs=c.adrg.d||[];var bestDRG=drgs[0];
    for(var j=0;j<drgs.length;j++){if(drgs[j].s===mccLevel){bestDRG=drgs[j];break;}}
    var isTop=t===0;

    // 如果 localBoost 不为0，尝试找校正后的 DRG
    var altDRG = null;
    if(localBoost !== 0 && drgs.length > 1){
      var targetLevel = localBoost === 1 ? 'MCC' : (localBoost === -1 ? 'noCC' : 'CC');
      for(var aj=0;aj<drgs.length;aj++){
        if(drgs[aj].s === targetLevel){altDRG = drgs[aj]; break;}
      }
    }
    if(altDRG) bestDRG = altDRG;

    // Path description
    var path='';
    if(c.mdcTier===0){
      if(catDisplay==='外科'||catDisplay==='操作')path='主手术→MDC'+c.md.mdc+' '+catDisplay+'组';
      else path='主诊→MDC'+c.md.mdc+' 内科组';
    }else if(c.mdcTier===1){
      path='QY: 主诊MDC'+dxMDC+'≠手术MDC'+pxMDC+' → 手术优先';
    }
    if(c.adrg.pdSpec>0)path+=' | 诊断匹配: '+(c.adrg.pdSpec===1?'前缀':'泛匹配');
    if(localBoost!==0 && altDRG){
      var boostLabel = localBoost===1 ? '本院历史倾向MCC' : '本院历史倾向noCC';
      path+=' | 📊 '+boostLabel;
    }

    html+='<div style="'+(isTop?'background:rgba(79,140,247,.06);border:2px solid #4f8cf7;':'background:#fff;border:1px solid #e8ebf0;')+'border-radius:8px;padding:10px 12px;margin-bottom:8px">';
    html+='<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px">';
    html+='<b style="font-size:1.1em;color:'+(isTop?'#4f8cf7':'#1a1d28')+'">'+bestDRG.c+'</b>';
    html+='<span style="font-size:.82em">'+bestDRG.n+'</span>';
    html+='<span style="font-size:.65em;padding:1px 6px;border-radius:8px;background:'+catColor+';color:#fff">'+catDisplay+'</span>';
    if(mccLevel!=='noCC')html+='<span style="font-size:.65em;padding:1px 6px;border-radius:6px;background:'+(mccLevel==='MCC'?'#e85555':'#e8963a')+';color:#fff">'+mccLevel+'</span>';
    if(isTop)html+='<span style="font-size:.65em;padding:1px 6px;border-radius:6px;background:#3ba06a;color:#fff">推荐</span>';
    html+='</div>';
    html+='<div style="font-size:.7em;color:#8b919e">ADRG '+c.adrg.a+' '+c.adrg.an+(c.md.mdc?' | MDC '+c.md.mdc+' '+c.md.mdc_n:'')+'</div>';
    if(path)html+='<div style="font-size:.7em;color:#4f8cf7;margin-top:2px">'+path+'</div>';
    if(c.adrg.matched.length)html+='<div style="font-size:.7em;color:#8b919e;margin-top:2px">匹配: '+c.adrg.matched.join(' / ')+'</div>';
    if(drgs.length>1){
      html+='<div style="font-size:.68em;color:#8b919e;margin-top:2px">变体: ';
      for(var v=0;v<drgs.length;v++){
        var dv=drgs[v],isDv=dv.s===mccLevel;
        html+='<span style="font-family:monospace;color:'+(isDv?'#4f8cf7':'#8b919e')+';'+(isDv?'font-weight:700':'')+'">'+dv.c+' ['+dv.s+']</span>'+((v<drgs.length-1)?' | ':'');
      }
      html+='</div>';
    }
    if(mccDx.length)html+='<div style="font-size:.7em;margin-top:2px"><span style="color:#e85555">MCC: '+mccDx.join(', ')+'</span></div>';
    if(ccDx.length)html+='<div style="font-size:.7em;margin-top:1px"><span style="color:#e8963a">CC: '+ccDx.join(', ')+'</span></div>';
    html+='</div>';
  }
  if(!html)html='<div style="font-size:.82em;color:#8b919e;padding:12px;text-align:center">未找到匹配的DRG分组</div>';
  return html;
}
